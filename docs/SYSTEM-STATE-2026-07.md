...
- **2026-09-03 — the module-gaps backlog (#336–#366) is part-built.** Landed: **#336, #338, #339,
  ...
  regression.
- **2026-09-03 — Round 2, a second parallel round of six.** #343 audit writers (`0f45c99`), #347
  report freshness + auto-escalations cube (`ddc2e05`), #353 inventory ledger closure (`989ba44`),
  #355 cross-zone page completion (`5ce3699`), #356 intra-day queue hygiene (`59c2096`), #360 SE poll
  contract (`042aa0b`). **21 of 31 slices are now done**, and #331/#333/#335 closed inside them.
  Round 2 also **changed the data layer**: `20260903150000_inventory_ledger_closure` relaxes
  `inventory_transactions.se_id` / `component_id` to nullable, adds `device_id` + FK + index and a
  CHECK that a row accounts for an engineer's stock **or** a device — see §2.5. Hand-written; the
  **drift gate cannot run on this box** and was not run.
- **Two corrections from Round 2 that change what published numbers mean.** (1) The efficiency cube's
  auto-escalations leg counted intra-day escalations on `updated_at AND status =
  'ESCALATION_REQUIRED'`; every resolver rewrites that row **in place** at resolution, so a resolved
  escalation matched neither the status nor the day it was raised and disappeared from every day. The
  metric read **lowest on the days managers cleared the queue fastest** — inverted, in the flattering
  direction. It now counts on the raising instant (`acceptance_deadline IS NULL`), matching the
  cross-zone leg. Days computed before #347 are **not** restated: `computeDay` is idempotent so a
  replay is safe, but the cron only ever computes the previous day, and `POST
  /reports/efficiency/recompute?day=` is the supported door for restating a range once someone
  decides how far back is worth restating. (2) Every `/reports/*` payload now carries `dataAsOf`, and
  the admin badge has a **real** 48h threshold (2× the daily cube cadence) — before #347 the badge
  compared the value against nothing and could never go stale.
- **Known surfacing gap, filed not deferred:** #360 rebuilt the SE poll contract but the mobile
...
| `intraday_insertions` | CRITICAL-insertion ledger — `ASSIGNED_DIRECT`/`ESCALATION_REQUIRED` written going forward (#268); `PENDING_ACCEPTANCE`/`ACCEPTED`/`DECLINED`/`TIMED_OUT` + `retry_chain` JSONB remain for historical rows only, nothing writes them any more. `offered_se_id`/`acceptance_deadline` nullable since #268 (an escalation with no candidate never had an SE to name). **`acceptance_deadline IS NULL` became load-bearing in #347**: it is the *only* surviving marker that a row was raised as an escalation — `status` is overwritten in place on resolution and `offered_se_id` is overwritten with whoever took the work (which is not who it was offered to; it was offered to nobody). The efficiency cube's auto-escalations leg predicates on it, so **a future writer must not create a deadline-less row for any other reason** without revisiting that query | partial unique `intraday_insertions_one_live_offer_per_ticket WHERE PENDING_ACCEPTANCE` (#101, `20260709120000`) — inert going forward, kept for the historical rows it still guards |
...
| `inventory_transactions` | Ledger: PRE_VERIFICATION→DEDUCTED/ROLLED_BACK; 409-loser SHADOW_USE→RECONCILED/DISPUTED. **#353 added the two closing rows and made the table two-grained.** `SHADOW_USE_DISPUTE_RESTORE` is the *compensating* row for a dispute — the DISPUTED row is the WM's decision and the ZM's queue, so it stays, and the restore is a readable fact rather than an unexplained `se_van_stock` increment. `RECOVERY_RECEIPT` is **device-keyed, not component-keyed**: a recovered device is not a SKU movement, and under INV-G2's assumed default it maps to no component and moves no stock (`writeRecoveryReceipt` in `recovery.service.ts` is the one place to reverse that). Hence `se_id` and `component_id` are **nullable** and `device_id` exists; every pre-#353 row still carries both. Readers must not assume `se_id`/`component_id` are present — `verification.service.ts` narrows rather than asserts | CHECK `qty > 0`; **CHECK `inventory_transactions_keyed` (`se_id IS NOT NULL OR device_id IS NOT NULL`)** — a row accounts for an engineer's stock or for a device, never neither; `device_id` FK → `devices` ON DELETE RESTRICT + index (`20260903150000_inventory_ledger_closure`, hand-written, **drift gate not run**) |
