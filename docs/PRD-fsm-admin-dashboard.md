...
54. As an Operations Head, I want to configure Company Tier (PLATINUM / GOLD / SILVER) and Company Priority Rank (A / B / C) per company, overriding CRM/SAP data when integration data is missing, so that the Recommender scores candidates correctly. (Extension #157, operator-approved 2026-07-23: Central Service Manager and Zonal Manager may additionally set **zone-scoped, expiring** tier overrides — mandatory reason, expiry ≤ 2 months, audited, auto-reverting — over this OH-owned global tier; the effective tier for a (company, zone) is the newest active unexpired override else the global tier.)
