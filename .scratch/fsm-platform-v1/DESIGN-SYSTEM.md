...
### 3.2 Grid
- **Desktop:** sidebar (fixed) + fluid content. Content column caps at `1920px` (`.enterprise-page`) with a **`12–20px` shell gutter** — deliberately tight (#168): the sidebar and top bar already frame the content, so a wide inner margin only starves the dense tables of the width an id or an IMSI needs to stay on one line. KPI strips are **4–6 equal columns** `[01=4/5, 04=4, 21=6]`. Two-column analytics/detail splits ≈ **2fr / 1fr** (detail: evidence left, Manager Controls right `[08,09]`; reports: chart left, breakdown right `[23,24]`).
...
### 5.7 Cards / PageHeader / TableToolbar / EmptyState / ErrorState / Skeleton / Toast / Tabs
- `PageHeader`: **no longer a visible card (#168).** Renders the title + subtitle `sr-only` (the
  top-bar breadcrumb is the visible page name since #160; the heading survives for assistive tech,
  which the breadcrumb's `aria-current` span cannot provide) plus a borderless right-aligned actions
  row. The prop signature is unchanged — call sites were not touched.
- `SectionCard`: white card, caps title, optional header action.
- `TableToolbar` (#168): the strip **inside** a table card holding that table's own controls — caps
  title left, filter controls flowing, `ml-auto` trailing slot for the download button. Reached via
  `DataTable`'s `toolbar` / `toolbarTitle` props, or used directly by the two bespoke `<table>`
  drill-downs. **A table's filters never live in a card of their own** `[07,10,21,22,26]`.
- `FilterBar`: demoted to a plain flex group for the few control clusters that are not a table's.
  `SearchInput` / `FilterSelect` are `h-8`; `FilterSelect` is fixed-width so a seven-filter toolbar
  does not wrap into ragged lines.
...
**Dark mode (#168) is the payoff of that last rule.** Because no component carries a raw hex, dark mode is a *re-point of the same token names* under `:root[data-theme="dark"]` — not a `dark:` sweep across the component tree. `:root[data-theme="dark"]` (specificity 0,2,0) outranks Tailwind's `:root` (0,1,0), so the override wins regardless of source order.

- The palette is a **pitch-black canvas with genuinely raised card surfaces** (2026-07-29 operator ask; it was a blue-cool near-black before): `surface-app #000000` → `surface-card #0d0d0d` → `raised #161616` → `sunken #1f1f1f` stays a monotonic ladder, and `chrome-*` (sidebar / footer / table header, `chrome-900 #171717`) sits *lighter* than the canvas — on a dark page an elevated rail reads as chrome where a darker one reads as a hole. Every step is **neutral grey, zero hue**: a few points of blue is enough to stop the canvas reading as black, which is the whole point of the ask. `--color-canvas-top` is also `#000000`, so the body gradient is flat black rather than a wash.
- Semantic foregrounds are lifted and sit **mid-scale**, because each must survive both as text on the dark card *and* as a fill with white text (`DataTable`'s `danger` active row is `bg-critical text-white`).
- `--color-row-hover` and `--color-canvas-top` exist so the table hover wash and the body gradient are theme-switchable rather than hard-coded in the base layer.
- **`--color-link` is the emphasis foreground** for a drill-down count or an inline link on a surface (the `845 / 10758` in Company/Plant Overview, per-bucket counts, "View all", the Critical+ scorecard count). It is a token distinct from `brand-700` because the two roles diverge in dark: `brand-700` must stay a deep crimson (it is the bottom of the primary button's gradient), whereas the same crimson as a *foreground* on the near-black card is ~1.9:1 — a table of them reads as a wall of unreadable dark red. Light `#8f0f24` (unchanged from before the split), **dark `#ffffff`**. `text-brand-700` survives only where it is text on a `brand-300` **fill** (`Badge`, `Toast`, the top-bar avatar) — a different role that reads correctly in both themes.
- **Ink runs brighter than a straight inversion.** On a black canvas a "white" that is really light grey reads as dimmed, so `ink-strong` is pure `#ffffff` and `ink` (`#e6e6e6`) sits just under it: table content is white first and grey only where it is genuinely secondary. Ink is neutral too — a blue-grey body text tints the whole page back off black.
- **Two `dark:` escapes only** (the top-bar avatar's tint/foreground pairing), where a token swap cannot express the change. Adding a third is a signal that a token is missing.
- Chart *chrome* (axis, grid, ticks, tooltip cursor) is `var(--color-…)`; chart *series* colours stay literal — they are semantic, must stay distinguishable from each other rather than from the background, and sit mid-scale so they read on both canvases.
- Ownership: `ThemeProvider` (`components/shell/ThemeContext.tsx`) owns `data-theme` from mount; `lib/applyStoredTheme.ts` sets the same attribute from `main.tsx` before React mounts so a dark reload does not flash light. Default preference is `system`, tracked live via `matchMedia`; an explicit choice pins and persists.
