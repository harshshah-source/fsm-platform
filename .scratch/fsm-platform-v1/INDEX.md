...
- 114 — `.gitignore` `data/` rule shadows the admin core UI source dir `apps/admin/src/components/data/` (MetricStrip/Toast/DataTable/PageHeader/…): whole dir untracked (`git ls-files` empty), so the branch tip **can't build alone** — likely root cause of that standing handoff note; commits touching `data/` silently drop files unless force-added → *(ready-for-agent — surfaced 2026-07-08 while committing the admin Run-Ingestion/rolling-KPI feature, which had to force-add its own `data/` files; `.gitignore` deliberately left untouched that session. Re-anchor the `data/`+`backups/` patterns, `git add` the un-ignored sources, sweep `apps/**`/`packages/**` for other shadowed dirs. Folds in a trivial cleanup: two pre-existing `zones.reduce is not a function` unhandled errors at `ZmDashboard.tsx:45` from the cross-zone/zm-scorecard route-gating tests' non-array stubs.)*

- 115 — `.gitignore` `docs/*` leaves the entire doc set (PRD/workflow/audits/architecture/UI refs) untracked — single-disk DR exposure; policy decision on what is versioned → *(ready-for-human — filed 2026-07-10 by the SYSTEM-STATE audit; same family as #114)*
