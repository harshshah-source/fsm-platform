...
- The palette is a **blue-cool near-black canvas with genuinely raised card surfaces**, not an inverted greyscale: `surface-app #0e1014` → `surface-card #16191f` → `raised #1c202a` → `sunken #232833` stays a monotonic ladder, and `chrome-*` (sidebar / footer / table header) sits *lighter* than the canvas — on a dark page an elevated rail reads as chrome where a darker one reads as a hole.
...
- **Ink runs brighter than a straight inversion.** On a near-black canvas a "white" that is really light grey reads as dimmed, so `ink-strong` is pure `#ffffff` and `ink` sits just under it: table content is white first and grey only where it is genuinely secondary.
