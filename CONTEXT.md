...
Cross-zone strategic owner; handles fleet-wide reporting, strategic cross-zone escalations, and acts as second-line backup when *both* the Zonal Manager and Central Service Manager are unavailable. Also the **system configurator** — owns zone/plant setup, SE mappings, SLA rules, priority rules, the global company tier (CSM/ZM may additionally set zone-scoped, expiring tier overrides — extension #157), and user account management. No separate "Admin" persona exists. **Current-phase access:** complete access across all modules and all zones; all powerful actions audited.
...
**Company Master**:
The reference table of all companies (`COMPANY_MASTER`). Each row carries `company_id`, `company_tier` (Platinum / Gold / Silver), `company_priority_rank` (A / B / C / …), and standard contact/contract fields. Sourced from CRM/SAP; Operations Head can override per-company. **Scoped tier overrides (#157):** the *global* `company_tier` is OH-owned, but Central Service Manager and Zonal Manager may additionally create **zone-scoped, expiring** tier overrides (mandatory reason ≥ 10 chars, expiry ≤ 2 months, every mutation audited, silent auto-revert at expiry) — an operator-approved extension (2026-07-23) of the previously OH-only authority. The **effective tier** for a `(company, zone)` is the newest ACTIVE, unexpired override, else this global tier; the 3-tier canon (**PLATINUM > GOLD > SILVER**) is unchanged.
...
## 19. The operating day is the IST calendar day; the daily dispatch run is operator-configurable, defaulting to 05:00 IST

**Decision.** "Today" means the **`Asia/Kolkata` calendar day, midnight to midnight**, everywhere a day is bucketed — Day Plans, work schedules, deferral gates, same-day override windows, and the day-scoped reads that feed them. Storage is unchanged and stays UTC `timestamptz` (backend LLD: *"IST is a display concern only"*); this decision governs **bucketing**, not storage, and the two were previously conflated. The **daily dispatch run** fires at **05:00 IST by default**, with `Asia/Kolkata` as the scheduler's business timezone — and the time is **operator-configurable from application settings by the Operations Head**, not hardcoded and not an environment variable. Leave and availability **dates** are IST calendar days: an SE who books 10 Aug off is unavailable for all of 10 Aug IST.

**Why this and not the alternative.** The system had been bucketing days by **UTC midnight**, which is **05:30 IST** — so everything a field engineer did between midnight and 05:29 IST was filed under the previous calendar day, in the Day Plan and in every report. That was never a decision: `utcDayStart` was consolidated as a refactor side-effect and roughly fifteen services inherited it. Rejected keeping UTC-and-documenting-it, because an Indian field operation whose day starts at 05:30 produces permanent operator confusion — a ticket raised at 01:00 IST belonging to "yesterday" is not explainable to the people using it, and the cost recurs daily rather than once. Rejected a split IST-user-facing / UTC-internal model as more machinery than the problem needs. The dispatch time is configurable rather than fixed because the right hour is an operational question that will want tuning during the pilot, and a redeploy is too high a bar for that.
