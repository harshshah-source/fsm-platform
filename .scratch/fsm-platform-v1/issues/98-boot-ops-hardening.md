...
- [ ] Boot aborts with a clear error when `JWT_ACCESS_SECRET` is unset, equals the old dev default, or is too short; and when `DATABASE_URL` is unset/unparseable. In `production` these are always fatal.
- [ ] `TokenService` has no `?? 'dev-...'` fallback; tokens can only be signed with the validated secret.
- [ ] `main.ts` calls `enableShutdownHooks()`; a SIGTERM triggers Prisma `$disconnect` and the MySQL pool `end` (assert via the `onModuleDestroy` chain). `bootstrap()` rejection logs a fatal and exits non-zero.
- [ ] `GET /api/health` returns 200 with a body indicating liveness; a readiness check reports DB reachability. Route is `@Public()`.
- [ ] A global exception filter is registered; unhandled errors return a sanitized JSON body (no stack), are logged with a correlation id, and 500s are no longer raw.
- [ ] Requests carry/emit a correlation id in logs.
- [ ] Existing e2e suite stays green; new tests cover env-validation failure modes and the health/readiness responses.
